...
size_t server_response_callback(char *buffer, size_t length, void *context)
{
    if(strlen(test_body_index) == 0) {
        test_body_index = test_body;
    }
    *buffer = *test_body_index;
    test_body_index++;
    return 1;
}
...
void server_callback(http_transaction_t transaction, void *context)
{
    ...
    http_response_set_body_length(transaction->response, strlen(test_body));
    transaction->response->body_callback = server_response_callback;
}
